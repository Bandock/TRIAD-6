...
TRIAD_6::Renderer::Renderer() : renderer_error_str(TRIAD_6::RendererError::NoError), GLContext(nullptr), PrimaryVertexShaderId(0), MachineFragmentShaderId(0), MainProgramId(0), VAOId(0), VBOId(0), IBOId(0), DisplayTextureId(0), display_buffer(315 * 195, { 0, 0, 0 })
{
	vertices = {
...
bool TRIAD_6::Renderer::Setup(SDL_Window &Window)
{
	...
	{
		renderer_error_str = RendererError::ExplicitAttribLocationNotSupported;
		return false;
	}
	if (!GLEW_ARB_texture_storage)
	{
		renderer_error_str = RendererError::TextureStorageNotSupported;
		return false;
	}
	...
	glTexStorage2D(GL_TEXTURE_2D, 1, GL_RGBA8, 315, 195);
	glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 315, 195, GL_RGBA, GL_UNSIGNED_BYTE, display_buffer.data());
